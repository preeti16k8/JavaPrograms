...
public class CountOfSubsetSumWithAGivenSum {
  ...
  //      return countOfSubSetSumWithGivenSum(set, set.length, sum);
        return countOfSubSetSumWithGivenSumBottomUp(set, sum);
    }
    ...
    private static int countOfSubSetSumWithGivenSum(int[] set, int n, int sum) {
        ...
        if (set[n-1] <= sum) {
             ...
             */
            return t[n][sum] = countOfSubSetSumWithGivenSum(set, n - 1, sum - set[n-1]) +  // Choosing the current item from set.
    ...
    private static int countOfSubSetSumWithGivenSumBottomUp(int[] set, int sum) {
