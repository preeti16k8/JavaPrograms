...
public class SubSetSumProblem {
 ...
 //       System.out.println("Recursive with Memorization  for Sum " + sum + " is " + result);
        ...
        System.out.println("Bottom Up Approach  " + isSubSetSumBottomUp(set, set.length, sum));
        return result;
    }
    ...
    public static boolean isSubSetSum(int set[], int n, int sum) {
        ...
        // Choice
        if (set[n-1] <= sum) { // Item is less<= to the Sum, so might be included
            return t[n][sum] = isSubSetSum(set, n - 1, sum - set[n-1]) || // Choose
